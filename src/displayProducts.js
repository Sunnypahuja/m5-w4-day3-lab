...
function DisplayProducts(props) {
  ...
  return (
    <div>
      <label className="ms-5">Sort by Price</label>
      <select
        id="mySelect"
        onChange={(e) => props.myFunction(products, e.target.value)}
      >
      ...
      <div>
        {products.map((product) => {
          return (
            <div className="card" key={product.id}>
              <h4 className="text-start mt-4 ms-5 me-4 mb-2">
                {product.desc}
                <span className="text-start mt-4 ms-5 me-4 mb-2">
                  $ {product.price}
                </span>
              </h4>
